...
if app.ENABLE_WAR_KILL_NOTICE:
	import guildwarkillcounter
if app.ENABLE_FAST_SOULSTONE_READ_SYSTEM:
	import uiruhtasi
...
class GameWindow(ui.ScriptWindow):
	def __init__(self, stream):
		if app.GLOBAL_RANKING_INFO:
			self.rankingWindow = None
		ui.ScriptWindow.__init__(self, "GAME")
		self.SetWindowName("game")
		net.SetPhaseWindow(net.PHASE_WINDOW_GAME, self)
		player.SetGameWindow(self)
		self.wipserMenu = uiWhisperAdmin.AdminWhisperManager()
	...
	def __BuildKeyDict(self):
		...
		onPressKeyDict[app.DIK_F12]	= lambda : self.showWisperMenu()
		...
		if app.ENABLE_HIDE_PREMIUM_EFFECTS:
			onPressKeyDict[app.DIK_J]		= self.__PressBlendKey
		...
		"""Open Switchbot on F5 press"""
		#onPressKeyDict[app.DIK_F5] = lambda : self.interface.ToggleSwitchbotWindow()
		onPressKeyDict[app.DIK_F5] = lambda : self.interface.ToggleWorldBoss()

		"""Open doungeons menu on F6"""
		onPressKeyDict[app.DIK_F6] = lambda : self.__pressKeyF6()

		if app.GLOBAL_RANKING_INFO:
			onPressKeyDict[app.DIK_F7] = lambda : self.OpenRanking()	
	...
	if app.ENABLE_HIDE_PREMIUM_EFFECTS:
		def __PressBlendKey(self):
			if app.IsPressed(app.DIK_LCONTROL) or app.IsPressed(app.DIK_RCONTROL):
				if 0 == interfaceModule.IsBlendHide:
					interfaceModule.IsBlendHide = 1
					systemSetting.SetPremiumAffect(True)
				else:
					interfaceModule.IsBlendHide= 0
	...
	def OpenRefineDialog(self, targetItemPos, nextGradeItemVnum, cost, prob, tip=0, tipreal=0):
		self.interface.OpenRefineDialog(targetItemPos, nextGradeItemVnum, cost, prob, tipreal)
		import dbg
		dbg.TraceError("{0}-TEST-{1}-{2}".format(int(tip), int(prob), int(tipreal)))
	...
	## Chat
	def OnRecvWhisper(self, mode, name, line, language):		### MULTILANGUAGE
		...
		chat.SetLanguageWhisper(name, language)		## MULTILANGUAGE
	...
	if app.ENABLE_PM_ALL_SEND_SYSTEM:
		def BINARY_OnRecvBulkWhisper(self, content):
			...
			self.interface.RegisterGameMasterName("[Admin Message]")
			chat.AppendWhisper(chat.WHISPER_TYPE_CHAT, "[Admin Message]", content)

			self.interface.RecvWhisper("[Admin Message]")

	if app.ENABLE_NEW_YANG_LOG_SYSTEM:
	...
	if app.ENABLE_NEW_EXP_LOG_SYSTEM:
		def OnRecvExp(self, exp):
			self.interface.OnRecvExpNew(exp)
	...
	if app.ENABLE_OPTIONS_HIDE_SYSTEM:
	...
	def OnMouseLeftButtonUp(self):
		...
		if mouseModule.mouseController.isAttached():
			...
			elif app.ENABLE_SPECIAL_STORAGE and player.SLOT_TYPE_UPGRADE_INVENTORY == attachedType or player.SLOT_TYPE_BOOK_INVENTORY == attachedType or player.SLOT_TYPE_STONE_INVENTORY == attachedType or player.SLOT_TYPE_BONUS_INVENTORY == attachedType or player.SLOT_TYPE_CHEST_INVENTORY == attachedType or player.SLOT_TYPE_COSTUME_INVENTORY == attachedType:
				self.__PutItem(attachedType, attachedItemIndex, attachedItemSlotPos, attachedItemCount, self.PickingCharacterIndex)
	...
	def __PutItem(self, attachedType, attachedItemIndex, attachedItemSlotPos, attachedItemCount, dstChrID):
		if player.SLOT_TYPE_INVENTORY == attachedType or player.SLOT_TYPE_DRAGON_SOUL_INVENTORY == attachedType:
			...
			player.SLOT_TYPE_BONUS_INVENTORY == attachedType or\
	...
	def RequestDropItem(self, answer):
		...
		if answer:
			...
			elif app.ENABLE_SPECIAL_STORAGE and player.SLOT_TYPE_UPGRADE_INVENTORY == dropType or player.SLOT_TYPE_BOOK_INVENTORY == dropType or player.SLOT_TYPE_STONE_INVENTORY == dropType or player.SLOT_TYPE_BONUS_INVENTORY == dropType or player.SLOT_TYPE_CHEST_INVENTORY == dropType or player.SLOT_TYPE_COSTUME_INVENTORY == dropType:
	...
	def OnUpdate(self):
		...
		if app.ENABLE_INVENTORY_ADDITION:
			if int(constInfo.menurutbe) == 1:
				self.OpenTitleSystem()
				constInfo.menurutbe=0
	...
	def __ServerCommand_Build(self):
		serverCommandList={
			...
			"worldBoss"				: self.RankingBoss,
			"worldCoins" : self.RecvWorldCoins,
			# END_OF_PRIVATE_SHOP_PRICE_LIST
			
		}
		if app.ENABLE_GUILD_MEETING:
			serverCommandList["loncatoplantisi"] = self.loncatoplantisi
		if app.ENABLE_WAR_KILL_NOTICE:
			serverCommandList["GW_Kill_Update"] = self.__GuildWar_ProcessKillInput
		if app.ENABLE_FAST_SOULSTONE_READ_SYSTEM:
			serverCommandList["ruhtasiekranac"] = self.hizliruhtasi
		if app.ENABLE_CHEQUE_SYSTEM:
		...
		if app.ENABLE_NEW_HIDE_COSTUME_SYSTEM:
		...
		if (app.ENABLE_CHEQUE_TICKET_SYSTEM):
	...
	if app.ENABLE_MODEL_RENDER_TARGET:
		def TestRenderTarget(self):
			self.RenderTarget = uiRenderTarget.RenderTarget()
	...
	if app.ENABLE_GUILD_MEETING:
		def loncatoplantisi(self):
			...
			questionDialogtoplanti.SetText("Ask to meeting?")
	...
	if app.ENABLE_NEW_PET_SYSTEM:
		def SetPetEvolution(self, evo):
			petname = ["Tinar", "Salbatic", "Curajos", "Erou"]
	...
	if app.ENABLE_SKILL_CHOOSE_SYSTEM:
	...
	if app.ENABLE_NEW_TARGET_UI:
		def OpenTargetNew(self, isPC, vid, hp, curHP, maxHP):
	...
	def BINARY_RefreshWorldBoss(self):
		self.interface.wndRanking.RefreshRanking()

	def RankingBoss(self, arg1, arg2, arg3):
		if self.interface and self.interface.wndRanking:
			self.interface.wndRanking.SetMyRank(arg1, arg2, arg3)
			
	def RecvWorldCoins(self, coins):
		constInfo.coinsWorldBoss = int(coins)
