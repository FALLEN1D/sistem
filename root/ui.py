...
class Button(Window):
	def __init__(self, layer = "UI"):
		...
		if app.ENABLE_DETAILS_INTERFACE:
			self.overFunc = None
			self.overArgs = None
...
class SlotWindow(Window):
	...
	if app.SIGN_FOR_PLUS_ITEMS:
		def AppendPlusOnSlot(self, slotNumber, xPos, yPos):
			wndMgr.AppendPlusOnSlot(self.hWnd, slotNumber, xPos, yPos)
...
class TitleBar(Window):
	...
	if app.ENABLE_DETAILS_INTERFACE:
		def CloseButtonHide(self) :
			self.imgRight.LoadImage("d:/ymir work/ui/pattern/titlebar_right_w_close.tga")
			self.btnClose.Hide()
...
class NewScrollBar(Window):
	...
	def CreateScrollBar(self):
		...
		barSlot.SetParent(self)
		barSlot.LoadImage("d:/ymir work/ui/pattern/scrollbar_middle.tga")
		barSlot.AddFlag("not_pick")
		barSlot.Show()
...
if app.ENABLE_REFINE_RENEWAL:
	class CheckBox(Window):
		...
		def CreateElements(self):
			...
			self.backgroundImage.LoadImage("d:/ymir work/ui/game/ekstra/refine/checkbox.tga")
			...
			self.checkImage.LoadImage("d:/ymir work/ui/game/ekstra/refine/checked.tga")
			self.checkImage.Hide()
...
def MakeOptionsCheckBox(parent, text, x, y):
    ...
    checkBox.SetBaseCheckImage("icon/options_checkbox.tga")
    checkBox.Show()
    return checkBox
...
def MakeTextWithoutAlign(parent, textlineText, x, y):
	textline = TextLine()
	if parent != None:
		textline.SetParent(parent)
	textline.SetPosition(x, y)
