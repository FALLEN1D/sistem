...
#ifdef __GLOBAL_RANKING_INFO__ 
		case HEADER_GC_RANKING_UPDATE:
			ret = RecvRankUpdate();
			break;
#endif

#ifdef ENABLE_WORLD_BOSS
			case HEADER_GC_WORLDBOSS:
				ret = RecvWorldBoss();
			break;
#endif
...
// UPDATE 11.05.21 -> done with crash when try to spam it.
{
	try
	{
		...
		while ((zPos = strChatBuf.find(strHyperItemLink, zPos)) != std::string::npos)
		{
			...
			if (!CItemManager::Instance().SelectItemData(dwItemVnum))
				return c_pszBuffer;
		...
		static char szFilteredChat[1024];
...
#ifdef ENABLE_TRADE_CHAT_SYSTEM
		case CHAT_TYPE_TRADE:	/* Ticaret Konu�mas?*/
#endif
			{
				char * p = strchr(buf, ':');
...
#ifdef ENABLE_MULTI_LANGUAGE_SYSTEM
		CPythonChat::Instance().AppendChat(kChat.type, FilterChat(buf), kChat.bEmpire);
#else
		CPythonChat::Instance().AppendChat(kChat.type, buf);
#endif
...
bool CPythonNetworkStream::RecvWhisperPacket()
{
	...
	if (CPythonChat::WHISPER_TYPE_CHAT == whisperPacket.bType || CPythonChat::WHISPER_TYPE_GM == whisperPacket.bType)
	{
		...
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OnRecvWhisper", Py_BuildValue("(issi)", (int)whisperPacket.bType, whisperPacket.szNameFrom, line, whisperPacket.bLanguage));
#else
...
#endif
#ifdef ENABLE_NEW_EXP_LOG_SYSTEM
		if (CItemData::POINT_EXP == PointChange.Type)
		{
			if (PointChange.amount > 0)
			{
	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
...
#endif
			{
				...
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "OpenTargetNew", Py_BuildValue("(iiLLL)", bIsPC, TargetPacket.dwVID, TargetPacket.bHPPercent, TargetPacket.iMinHP, TargetPacket.iMaxHP));
#else
				...
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard", Py_BuildValue("(iLLLi)", TargetPacket.dwVID, TargetPacket.bHPPercent, TargetPacket.iMinHP, TargetPacket.iMaxHP, TargetPacket.bElement));
#else
...
#else
#ifdef ENABLE_VIEW_TARGET_DECIMAL_HP
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard", Py_BuildValue("(iLLL)", TargetPacket.dwVID, TargetPacket.bHPPercent, TargetPacket.iMinHP, TargetPacket.iMaxHP));
#else
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard", Py_BuildValue("(iL)", TargetPacket.dwVID, TargetPacket.bHPPercent));
#endif
#endif
			else
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "CloseTargetBoard", Py_BuildValue("()"));
...
#ifdef ENABLE_GOLD_LIMIT_REWORK
	TraceError(" >> RecvRefineInformationPacketNew(pos=%d, result_vnum=%d, cost=%lld, prob=%d, type=%d)\n",
#else
	TraceError(" >> RecvRefineInformationPacketNew(pos=%d, result_vnum=%d, cost=%d, prob=%d, type=%d)\n",
#endif
		kRefineInfoPacket.pos,
		kRefineInfoPacket.refine_table.result_vnum,
		rkRefineTable.cost,
...
//#ifdef _DEBUG
//	char szBuf[256+1];
//	_snprintf(szBuf, sizeof(szBuf), "ĳ���� Ÿ���� ���� �Ǿ����ϴ� [%d:%s:%d]", kTargetCreate.lID, kTargetCreate.szTargetName, kTargetCreate.dwVID);
//	CPythonChat::Instance().AppendChat(CHAT_TYPE_NOTICE, szBuf);
...
static const auto DiscordClientID = "834500751148711946"; //Change
...
#ifdef __GLOBAL_RANKING_INFO__
bool CPythonNetworkStream::RecvRankUpdate()
{
	TPacketRank p;
	if (!Recv(sizeof(p), &p))
		return false;
	for (DWORD j = 0; j < 10; j++) {
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankLevel", Py_BuildValue("(isii)", j, p.m_ranking_l[j].name, p.m_ranking_l[j].empire, p.m_ranking_l[j].value));
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankDestroyedStone", Py_BuildValue("(isii)", j, p.m_ranking_d_s[j].name, p.m_ranking_d_s[j].empire, p.m_ranking_d_s[j].value));
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankKillMonster", Py_BuildValue("(isii)", j, p.m_ranking_k_m[j].name, p.m_ranking_k_m[j].empire, p.m_ranking_k_m[j].value));
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankKillBoss", Py_BuildValue("(isii)", j, p.m_ranking_k_b[j].name, p.m_ranking_k_b[j].empire, p.m_ranking_k_b[j].value));
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankCompletedDungeon", Py_BuildValue("(isii)", j, p.m_ranking_c_d[j].name, p.m_ranking_c_d[j].empire, p.m_ranking_c_d[j].value));
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankPlaytime", Py_BuildValue("(isii)", j, p.m_ranking_p[j].name, p.m_ranking_p[j].empire, p.m_ranking_p[j].value));
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankGold", Py_BuildValue("(isiL)", j, p.m_ranking_y[j].name, p.m_ranking_y[j].empire, p.m_ranking_y[j].value));
		//PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankGold", Py_BuildValue("(isii)", j, p.m_ranking_y[j].name, p.m_ranking_y[j].empire, p.m_ranking_y[j].value));
#ifdef GAYA_SYSTEM
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankGaya", Py_BuildValue("(isii)", j, p.m_ranking_g[j].name, p.m_ranking_g[j].empire, p.m_ranking_g[j].value));
#endif
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankCaughtFishes", Py_BuildValue("(isii)", j, p.m_ranking_c_f[j].name, p.m_ranking_c_f[j].empire, p.m_ranking_c_f[j].value));
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RankOpenedChest", Py_BuildValue("(isii)", j, p.m_ranking_o_c[j].name, p.m_ranking_o_c[j].empire, p.m_ranking_o_c[j].value));
	}
	return true;
}
#endif
...
#ifdef ENABLE_WORLD_BOSS
bool CPythonNetworkStream::RecvWorldBoss()
{
	TPacketWorldBoss packet;
	if (!Recv(sizeof(packet), &packet))
		return false;

	CPythonPlayer::Instance().ClearWorldBossMap();

	packet.wSize -= sizeof(packet);
	while (packet.wSize > 0)
	{
		TWorldTablePacket info;
		if (!Recv(sizeof(info), &info))
			return false;

		CPythonPlayer::Instance().SetWorldMap(info);
		 
		packet.wSize -= sizeof(info);
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_RefreshWorldBoss", Py_BuildValue("()"));

	return true;
}
void CPythonPlayer::SetWorldMap(TWorldTablePacket& crInfo)
{
	const std::string player = crInfo.szName;

	const auto it = worldBossRank.find(player);
	if (it != worldBossRank.end())
		it->second = crInfo;
	else
		worldBossRank.insert(std::make_pair(crInfo.szName, crInfo));
}
#endif
